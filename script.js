...
const playSound = s => { s.currentTime = 0; s.play().catch(e => console.warn("Audio play blocked")); };
...
document.querySelectorAll('.mode-select button').forEach(b => {
    b.onclick = () => { if (active) return; document.querySelectorAll('.mode-select button').forEach(x => x.classList.remove('active')); b.classList.add('active'); mode = b.dataset.mode; }
});
...
function startGlobalTimer() { clearInterval(globalInterval); timerEl.innerText = formatTime(globalTime); globalInterval = setInterval(() => { if (!active) return; globalTime--; timerEl.innerText = formatTime(globalTime); if (globalTime <= 10) timerEl.classList.add('danger'); if (globalTime <= 0) finish(); }, 1000); }
function startQuestionTimer() { clearInterval(questionInterval); questionTime = 60; qTimerEl.innerText = questionTime; questionInterval = setInterval(() => { questionTime--; qTimerEl.innerText = questionTime; if (questionTime === 5) { feed.innerText = "⚠️ SYSTEM WARNING: 5s LEFT"; playSound(sounds.timeout); } if (questionTime <= 0) { clearInterval(questionInterval); markWrong(); } }, 1000); }

// ===== QUESTIONS =====
function rand(min, max) { return Math.floor(Math.random() * (max - min + 1)) + min; }
function generateQuestion() {
    let a, b, c; if (mode === "easy") { a = rand(10, 30); b = rand(1, 20); c = rand(0, 10); }
...
// ===== STAR =====
function spawnStar() {
    const star = document.createElement('div'); star.className = 'star-fly';
    star.style.left = `${Math.random() * (window.innerWidth - 20)}px`;
    star.style.top = `${window.innerHeight - 50}px`; document.body.appendChild(star); setTimeout(() => star.remove(), 1000);
}
...
function markCorrect() {
    ...
    playSound(sounds.correct); feed.innerText = `ACCESS GRANTED [STREAK: x${streak}]`; box.classList.add('correct'); setTimeout(() => box.classList.remove('correct'), 300);
    spawnStar(); solved++; setTimeout(nextQuestion, 600);
}

function markWrong() { wrong++; streak = 0; solved++; playSound(sounds.wrong); feed.innerText = "ACCESS DENIED - DATA MISMATCH"; reveal.innerText = `EXPECTED_VAL: ${currentAnswer}`; box.classList.add('wrong'); setTimeout(() => box.classList.remove('wrong'), 300); setTimeout(nextQuestion, 1200); }
...
function finish() {
    ...
    clearInterval(globalInterval); clearInterval(questionInterval); playSound(sounds.finish);
    inEl.disabled = true; inEl.value = ''; btn.innerText = "DONE"; btn.style.opacity = "0.5"; btn.style.pointerEvents = "none";
    ...
    const logList = document.getElementById('log-list'); logList.innerHTML = `<div style="margin-bottom:10px;color:#666;">--- SYSTEM LOG ---</div>`; logList.innerHTML += correctTimes.map((t, i) => `OP_${i + 1}: <span style="color:#00ff41">OK (${t}s)</span>`).join('<br>');
    const rewardEl = document.createElement('div'); rewardEl.style.marginTop = "10px"; rewardEl.style.borderTop = "1px solid #333"; rewardEl.style.paddingTop = "10px"; rewardEl.innerHTML = `<div style="font-size:0.7rem;color:#888;">FINAL SCORE</div><div style="font-size:1.4rem;color:#00ff41;font-weight:800;">POINTS : ${points.toLocaleString()}</div>`;
    let stars = Math.round((correct / maxQuestions[mode]) * 3); stars = stars > 3 ? 3 : stars; rewardEl.innerHTML += `<div style="font-size:1.2rem;letter-spacing:5px;color:gold;">${'★'.repeat(stars) + '☆'.repeat(3 - stars)}</div>`; logList.appendChild(rewardEl);
